...
void  imu660ra_get_gyro(void); // 陀螺仪数据积分函数声明
...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    imu963ra_get_acc();                            // 读取加速度数据并积分函数
    imu660ra_get_gyro();                            // 陀螺仪数据积分函数
...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    Data_encoder();                            // 读取编码器数据并积分函数
    Data_Servo_encoder();                            // 读取伺服编码器数据并积分函数

    if (GyroOffset_init)
    {
        ICM_getEulerianAngles();                            // 陀螺仪数据积分函数
    }
   Nag_System();                            // 陀螺仪数据积分函数
   key_scanner();                           // 按键扫描函数
   car_start();                            // 启动函数
   if(telecontrol_flag = 1)
   {
        remote_use();                            // 遥控使用函数
   }
   else
